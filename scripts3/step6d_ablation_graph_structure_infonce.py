...
from step3_model_infonce import (PreciseADR, PreciseADRLoss, NodeFeatureProjection,
                          PatientNodeAugmentation, ADRPredictor,
                          build_model_from_graph)
from step4_training_infonce import evaluate, evaluate_all_metrics, convert_to_serializable
...
class PatientMLP(nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict=None, patient_mask=None):
        ...
        if patient_mask is not None:
            x = x[patient_mask]
        # Pass through all but last layer to get hidden rep
        h = x
        for layer in list(self.net.children())[:-1]:
            h = layer(h)
        logits  = self.net[-1](h)
        h_aug   = self.aug(h)
        return logits, h, h_aug
...
def _eval_on_variant(model, g: HeteroData, device: torch.device) -> dict:
    ...
    with torch.no_grad():
        ...
        logits, _, _ = model(x_dict, ei_dict, patient_mask=mask)
...
def run_graph_structure_ablation(variant: str, graph_path: str,
    ...
    cfg = dict(MODEL)
    metrics_p = os.path.join(out_dir, "final_metrics_infonce.json")
    ...
    out_path = os.path.join(out_dir, "ablation_graph_structure_infonce.json")
