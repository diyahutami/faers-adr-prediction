...
from step4_training_infonce import evaluate_all_metrics, train_model, convert_to_serializable
...
def run_demographic_ablation(variant: str, graph_path: str,
    ...
    for name, (pg, pa) in perturbations.items():
        ...
        with torch.no_grad():
            ...
            logits, _, _ = model(x_dict, ei_dict, patient_mask=tmask)
    ...
    out_json = os.path.join(out_dir, "ablation_demographic_infonce.json")
    with open(out_json, "w") as f:
        json.dump(convert_to_serializable(all_results), f, indent=2)

    pd.DataFrame(heatmap_rows).to_csv(
        os.path.join(out_dir, "ablation_demographic_auc_heatmap_infonce.csv"), index=False)
