...
Augmentation Layer and the InfoNCE contrastive loss?

Variants:
  CL-1  Full PreciseADR (HGT + augmentation + InfoNCE)       — reference
  CL-2  Without augmentation (noise, but no dedicated Aug FC)
  CL-3  Without InfoNCE (α = 0, Focal Loss only)
  CL-4  Without both augmentation and InfoNCE
...
from step3_model_infonce import (PreciseADR, PreciseADRLoss,
                          PatientNodeAugmentation, build_model_from_graph)
from step4_training_infonce import evaluate, evaluate_all_metrics, train_model, convert_to_serializable
...
class NoAugPreciseADR(PreciseADR):
    ...
    def forward(self, x_dict, edge_index_dict, patient_mask=None):
        ...
        # No augmentation: h_aug == h_patient (identity)
        h_aug  = h_patient
...
def build_variant(variant_id: str, train_graph, cfg: dict,
    ...
    alpha=0 disables InfoNCE in the loss (CL-3, CL-4).
...
def run_contrastive_ablation(variant: str, graph_path: str,
    ...
    for cl_id in ("CL-1", "CL-2", "CL-3", "CL-4"):
        ...
        if cl_id in ("CL-1", "CL-2"):
            ...
            # Save heatmap
            with open(os.path.join(out_dir, f"ablation_contrastive_grid_{cl_id.lower()}_infonce.json"), "w") as f:
                json.dump(convert_to_serializable(heatmap), f, indent=2)
        else:
            best_alpha = 0.0   # InfoNCE disabled
            best_tau   = cfg["tau"]
    ...
    out_path = os.path.join(out_dir, "ablation_contrastive_infonce.json")
...
def main():
    ...
    if args.seed is not None:
        from step4_training_infonce import set_seed
        set_seed(args.seed)
