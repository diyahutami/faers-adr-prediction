...
from config_2018 import (
    DATA_PATH, PREPROCESSED_PATH, OUTPUT_PATH, GRAPH_PATH, FAERS_SEP, COL,
    AGE_BINS, AGE_LABELS, MODEL, ADR_FILTER, GRAPH,
...
def _load_prop_analysis(data_path: str) -> pd.DataFrame | None:
    path = os.path.join(data_path, "PROPORTIONATE_ANALYSIS.txt")
...
class AERGraphBuilder:
    ...
    def load(self):
        ...
        self.prop   = _load_prop_analysis(self.data_path)
        print("  ✓ Tables loaded")
    ...
    def _drug_features(self) -> torch.Tensor:
        """One-hot over DrugBank_ID (or RXAUI) index."""
        n = len(self.drug_vocab)
        return torch.eye(n, dtype=torch.float)

    def _disease_features(self) -> torch.Tensor:
        n = len(self.disease_vocab)
        return torch.eye(n, dtype=torch.float)

    def _adr_features(self) -> torch.Tensor:
        n = len(self.adr_vocab)
        return torch.eye(n, dtype=torch.float)
