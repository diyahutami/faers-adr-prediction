...
def run_one_epoch(model: PreciseADR, graph, loss_fn: PreciseADRLoss,
    ...
    if scaler is not None and is_train:
        with autocast(device_type=device.type, dtype=torch.float16):
            # V2 model returns: logits, h_orig, z1, z2
            logits, h_orig, z1, z2 = model(x_dict, edge_idx_dict, patient_mask=mask)
            # V2 loss takes: logits, targets, h_orig, z1, z2
            total, l_focal, l_nce = loss_fn(logits, y, h_orig, z1, z2)
    ...
    return {
        ...
        "focal"  : l_focal.item(),
        "supcon": l_nce.item(),  # V2: SupCon instead of InfoNCE
    }
...
def evaluate(model: PreciseADR, graph, device: torch.device) -> dict:
    ...
    with torch.no_grad():
        ...
        # V2 model returns: logits, h_orig, z1, z2
        logits, _, _, _ = model(x_dict, edge_idx_dict, patient_mask=mask)
...
def train_model(train_graph, val_graph, model: PreciseADR,
                ...
                similarity_threshold: float = 0.3) -> tuple[PreciseADR, dict, dict]:
    """
    ...
    Args:
        ...
        similarity_threshold: Jaccard similarity threshold for SupCon positive pairs
    """
    ...
    loss_fn   = PreciseADRLoss(alpha=alpha, tau=tau, gamma=gamma, 
                               similarity_threshold=similarity_threshold).to(device)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if epoch % 10 == 0 or patience_left == 0:
            print(f"  Epoch {epoch:3d} | loss={train_info['loss']:.4f} "
                  f"focal={train_info['focal']:.4f} supcon={train_info['supcon']:.4f} "
...
def grid_search(train_graph, val_graph, base_model_cfg: dict,
    ...
    for alpha, tau in itertools.product(alpha_range, tau_range):
        ...
        trained, val_metrics, _ = train_model(
            ...
            similarity_threshold=base_model_cfg.get("similarity_threshold", 0.3),
        )
        auc = val_metrics["AUC"]
...
def main():
    ...
    with open(os.path.join(args.results_path, "training_summary_supcon.json"), "w") as f:
