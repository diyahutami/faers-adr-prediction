...
from step3_model_infonce import build_model_from_graph, PreciseADRLoss
from step4_training_infonce import evaluate, convert_to_serializable
...
def train_with_subset_mask(train_graph, val_graph, sampled_mask,
    ...
    for epoch in range(cfg["max_epochs"]):
        model.train()
        opt.zero_grad()
        logits, h, h_aug = model(x_dict, ei_dict, patient_mask=sampled_mask.to(device))
        loss, _, _ = loss_fn(logits, y_sub, h, h_aug)
...
def run_data_volume_ablation(variant: str, graph_path: str,
    ...
    metrics_path = os.path.join(results_path, variant, "final_metrics_infonce.json")
    cfg = dict(MODEL)
    ...
    # Save JSON
    out_json = os.path.join(out_dir, "ablation_data_volume_infonce.json")
    ...
    pd.DataFrame(rows).to_csv(
        os.path.join(out_dir, "ablation_data_volume_summary_infonce.csv"), index=False)
    return all_results
