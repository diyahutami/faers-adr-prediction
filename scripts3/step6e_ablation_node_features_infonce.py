...
from step6c_ablation_demographic_infonce import auc_per_adr, _load_related_adr_indices
...
def _train_and_eval(train_g: HeteroData, val_g: HeteroData, test_g: HeteroData,
    ...
    for ep in range(cfg["max_epochs"]):
        ...
        if ep % 5 == 0:
            ...
            with torch.no_grad():
                ...
                vl, _, _ = model(vx, vei, patient_mask=vm)
    ...
    with torch.no_grad():
        ...
        tl, _, _ = model(tx, tei, patient_mask=tm)
...
def run_node_feature_ablation(variant: str, graph_path: str,
    ...
    out_path = os.path.join(out_dir, "ablation_node_features_infonce.json")
